...
area (Circle _ r) = 2 * pi * r * r
area (Triangle a b c) = sqrt (p * (p - x) * (p - y) * (p - z))
  where
    p = perimeterT / 2
    perimeterT = perimeter (Triangle a b c)
    x = dist (a, b)
    y = dist (b, c)
    z = dist (c, a)
\end{code}
...
\begin{code}
numInTree :: IntTree -> Int
numInTree ILeaf = 0
numInTree (INode _ l r) = 1 + (numInTree l) + (numInTree r)
\end{code}
...
intListToTree (ICons x xs) = intListToTree' (ICons x xs) ILeaf
  where
    intListToTree' INil tree = tree
    intListToTree' (ICons x xs) tree = intListToTree' xs (insertInTree x tree)

insertInTree :: Int -> IntTree -> IntTree
insertInTree x ILeaf = (INode x ILeaf ILeaf)
insertInTree x (INode y treeL treeR)
  | x > y = (INode y treeL (insertInTree x treeR))
  | x < y = (INode y (insertInTree x treeL) treeR)
...
\begin{code}
-- foi retirado a implementação de Client anterior
data Client = 
  Customer {
    name :: Name
    , surname :: Surname
...
wantsOffers :: [Client] -> [Client]
wantsOffers clients = [x | x <- clients, offers x]
\end{code}